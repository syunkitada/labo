# QEMU

- [公式](https://www.qemu.org/)
- [公式: ドキュメント](https://www.qemu.org/docs/master/)

## 学習エントリーポイント

- 基礎
  - [2023: VA Linux エンジニアブログ: Qemu のしくみ (の一部)](https://www.valinux.co.jp/blog/entry/20230112)
- KVM
  - [2025: VA Linux エンジニアブログ: 新 Linux カーネル解読室 - KVM (概要)](https://www.valinux.co.jp/blog/entry/20251204)
- VirtIO/Vhost
  - [2025: Oracle Linux Blog: Introduction to VirtIO, Part 2: Vhost](https://blogs.oracle.com/linux/introduction-to-virtio-part-2-vhost)
- 脇道
  - [2023: VA Linux エンジニアブログ: Qemu 小ネタ集](https://www.valinux.co.jp/blog/entry/20230608)

## 使い方

- qemu の直接起動
  - [第 12 章 qemu-kvm を利用した仮想マシンの実行](https://manual.geeko.jp/ja/cha.qemu.running.html)
  - [qemu: 起動オプション一覧](https://manpages.debian.org/testing/qemu-system-x86/qemu-system-x86_64.1.en.html)
- qemu モニタの利用方法
  - [第 13 章 QEMU モニタを利用した仮想マシンの管理](https://manual.geeko.jp/ja/cha.qemu.monitor.html)
